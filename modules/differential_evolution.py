...
class DifferentialEvolutionSolver(object):
    ...
    def __init__(self, func, bounds, args=(),
        ...
        self.parameter_count = np.size(self.limits, 1) #10
        ...
        self._nfev = 0
        if isinstance(init, string_types): # initがstring_types
            if init == 'latinhypercube': # 母集団　初期化の散らばり方
    ...
    def init_population_lhs(self):
        ...
        # We need to do this sampling for each parameter.　　　　　　　　　候補解？
        samples = (segsize * rng.random_sample(self.population_shape)

        # Offset each segment to cover the entire parameter range [0, 1)　等間隔
                    + np.linspace(0., 1., self.num_population_members,
                                    endpoint=False)[:, np.newaxis])

        # Create an array for population of candidate solutions.　候補解の型に0をうめた何か
        self.population = np.zeros_like(samples)

        # Initialize population of candidate solutions by permutation of the
        # random samples.
        for j in range(self.parameter_count): #10
            order = rng.permutation(range(self.num_population_members))
            self.population[:, j] = samples[order, j]

        # reset population energies   全部infにしてる、、
...
##
    ...
    def solve(self):
        ...
        for nit in range(1, self.maxiter + 1):
            ...
            intol = (np.std(self.population_energies) <=
                        self.atol +
                        self.tol * np.abs(np.mean(self.population_energies))) #boolean型　これがTrueになったら収束
        ...
        DE_result = OptimizeResult( # x:最適化の解 fun:目的関数 #nit:試行回数
            x=self.x,
        ...
        if self.polish:
            result = minimize(self.func, #funcの最小値,そのパラメータ
                                np.copy(DE_result.x),
                                method='L-BFGS-B',
    ...
    def _mutate(self, candidate):
        ...
        rng = self.random_number_generator #擬似乱数
